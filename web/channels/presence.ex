defmodule LeanCoffee.Presence do
  @moduledoc """
  ...
  Presences can be tracked in your channel after joining:
      ...
      defmodule LeanCoffee.MyChannel do
        use LeanCoffee.Web, :channel
        alias LeanCoffee.Presence
  ...
  """
  use Phoenix.Presence, otp_app: :lean_coffee,
                        pubsub_server: LeanCoffee.PubSub
  import Ecto.Query
  alias LeanCoffee.{User,Repo}
  ...
  def fetch(_topic, entries) do
    ...
    for {key, %{metas: metas}} <- entries, into: %{} do
      ...
      {key, %{metas: metas, user: User.display_name(user)}}
    end
  end
end
